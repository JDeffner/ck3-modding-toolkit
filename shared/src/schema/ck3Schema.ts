 ...
 */
import type { Ck3SchemaEntry, RefField } from "./types";
import { STRUCTURES } from "./structures";
import { AMBIENT_SCOPES } from "./ambientScopes";

const CK3_SCHEMA_BASE: Ck3SchemaEntry[] = [
 ...
 */
export const CK3_SCHEMA: Ck3SchemaEntry[] = CK3_SCHEMA_BASE.map((entry) => {
...
/**
 * Ref kinds for key families too open-ended to enumerate in REF_FIELDS:
 * per-scope flag triggers/effects (`has_character_flag`, `remove_house_flag`, …
 * → flags declared at `add_*_flag` sites) and list membership (`is_in_list`,
 * the `list =` key of in-list iterators → names declared at `add_to_list`).
 */
